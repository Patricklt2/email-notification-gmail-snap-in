...
describe('resolveArtifactAttachments', () => {
  ...
  it('returns empty array when no ids provided', async () => {
    const out = await resolveArtifactAttachments(makeEvent(), [], console as unknown as GmailSnapInLogger);
    expect(out).toEqual([]);
  });
  ...
  it('blocks .eml attachments', async () => {
    ...
    await expect(
      resolveArtifactAttachments(makeEvent(), ['art:1'], console as unknown as GmailSnapInLogger)
    ).rejects.toThrow(/\.eml/i);
  });
  ...
  it('sanitizes filenames to prevent path traversal', async () => {
    ...
    const out = await resolveArtifactAttachments(makeEvent(), ['art:1'], console as unknown as GmailSnapInLogger);
    expect(out[0]?.filename).toBe('.._a.png');
  });

  it('enforces max attachments count', async () => {
    const ids = Array.from({ length: 11 }, (_, i) => `art:${i + 1}`);
    await expect(resolveArtifactAttachments(makeEvent(), ids, console as unknown as GmailSnapInLogger)).rejects.toThrow(
      /At most 10/
    );
  });
});
